...
export default function Partners({ logos }: PartnersProps) {
  ...
  // Helper function to get custom styling for specific logos
  ...
  // Helper function to get custom classes for specific logos
  const getLogoClasses = (src: string): string => {
    if (src.includes('amway-logo')) {
      return 'h-24 w-44';  // Larger size for Amway logo
    }
    if (src.includes('rorito-logo')) {
      return 'h-20 w-36 bg-blend-multiply';
    }
    return 'h-20 w-36';  // Default size for other logos
  };

  return (
    <section className="py-20 relative overflow-hidden bg-white">
      {/* Simple gradient fades for smooth scroll effect */}
          ...
          {doubledLogos.map((logo, index) => (
            ...
            >
              <img
                ...
                loading="eager"  // Ensure logos load immediately
              />
            </motion.div>
          ))}
        </motion.div>
